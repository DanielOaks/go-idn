...
const format_a1 = "\tTableElement{ 0x%06s, 0x%06s, d{} }, \t\t\t\t /* %s  */\n"
const format_b = "\tTableElement{ 0x%06s, 0, d{} },\t\t\t\t /* %s */\n"
...
func loadChars() {
	...
	resp, _, err := http.Get(*rfc3454URL)
	if err != nil {
		die.Log(err)
	}
	if resp.StatusCode != 200 {
		die.Log("bad GET status for rfc3454.txt", resp.Status)
	}
		...
		if line == "" {
			...
			if "A.1" == t {
				if len(line) == 4 || len(line) == 5 {
					fmt.Printf(format_a1, line, line, line)
					...
					fmt.Printf(format_a1, lo, hi, line)
				}
			} else if "B.1" == t {
				var char string = line[0:4]
				fmt.Printf("\tTableElement { 0x%06s, 0, d{}}, \t\t\t\t /* %s */\n", char, line)
			} else if "B.2" == t || "B.3" == t {
				var subs []string = strings.SplitN(line, ";", 0)
				var char = subs[0]
				var maps []string = strings.Fields(subs[1])
				...
				//fmt.Printf(format_b, lo, hi , line)
			} else if "C.1.1" == t || "C.1.2" == t {
				fmt.Printf(format_a1, line[0:4], line[0:4], line)
				...
				if len(chars) == 4 || len(chars) == 5 {
					fmt.Printf(format_a1, chars, chars, line)
