...
var die = log.New(os.Stderr, "", log.Lshortfile)
...
func BenchmarkNFKC(b *testing.B) {
	b.StopTimer()
	str := "ТрегункоtobiaswaqasТрегубенкоtobiaswaqasТрекоtobiaswaqasТрегубobiaswaqas"
...
func TestNKFC(t *testing.T) {
	...
	if resp.StatusCode != 200 {
		log.Exitf("bad GET status for NormalizationTest.txt", resp.Status)
	}
	...
	for i := 0; true; i++ {
		...
		if strings.HasPrefix(line, "#") || strings.HasPrefix(line, "@Part") {
			...
			c1 := NFKC(test.C1)
			if !reflect.DeepEqual(test.C4, c1) {
				t.Errorf("NormalizeNFCK(%v) == %v; want %v \t\t C1 - Line: %v", hex32(test.C1), hex32(c1), hex32(test.C4), i+1)
			}
			c2 := NFKC(test.C2)

			if !reflect.DeepEqual(test.C4, c2) {
				t.Errorf("NormalizeNFCK(%v) == %v; want %v \t\t C2 - Line: %v", hex32(test.C2), hex32(c2), hex32(test.C4), i+1)
			}
			c3 := NFKC(test.C3)
			if !reflect.DeepEqual(test.C4, c3) {
				t.Errorf("NormalizeNFCK(%v) == %v; want %v \t\t C3 - Line: %v", hex32(test.C3), hex32(c3), hex32(test.C4), i+1)
			}

			c4 := NFKC(test.C4)

			if !reflect.DeepEqual(test.C4, c4) {
				t.Errorf("NormalizeNFCK(%v) == %v; want %v \t\t C4 - Line: %v", hex32(test.C4), hex32(c4), hex32(test.C4), i+1)
			}

			c5 := NFKC(test.C5)
			if !reflect.DeepEqual(test.C4, c5) {
				t.Errorf("NormalizeNFCK(%v) == %v; want %v \t\t C5 - Line: %v", hex32(test.C5), hex32(c5), hex32(test.C4), i+1)
			}
		}
